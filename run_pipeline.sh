...
echo "$(date) STARTING pipeline (PID $$)" >> logs/cron.log

# Log initial resource usage
echo "RSS: $(ps -o rss= -p $$)kB, FDs: $(ls /proc/$$/fd | wc -l)" >> logs/cron.log

# Print Python info for debugging
echo "$(date) Using Python: $(which python)" >> logs/cron.log
echo "PATH: $PATH" >> logs/cron.log
...
(
    ...
    flock -n 9 || { echo "$(date) Pipeline already running, skipping this run" >> logs/cron.log; exit 0; }
    ...
    # Log exit code
    echo "$(date) EXIT code $EXIT_CODE" >> logs/cron.log
    
    # Log final resource usage
    echo "Final RSS: $(ps -o rss= -p $$)kB, FDs: $(ls /proc/$$/fd | wc -l)" >> logs/cron.log
    
    # Alert on non-zero exit code
    if [ $EXIT_CODE -ne 0 ]; then
        echo "WARNING: Pipeline failed with exit code $EXIT_CODE" >> logs/cron.log
    fi
